//! JSON writer: serde_json's serializer, extended to write `NaN` and `Infinity` and to take raw
//! number strings. Port of upstream `serializers/ser.rs` (itself adapted from serde_json).
    ...
    F: Formatter,
{
    ...
    fn serialize_str(self, value: &str) -> Result<Self::Ok> {
        format_escaped_str(&mut self.writer, &mut self.formatter, value).map_err(|e| {
        ...
        match self {
            Compound::Map { ser, state } => {
                ...
                *state = State::Rest;
                ...
                tri!(ser.formatter.end_array_value(&mut ser.writer).map_err(|e| {
                    PythonSerializerError {
                        message: e.to_string(),
                    }
                }));
    ...
    fn end(self) -> Result<()> {
        match self {
            Compound::Map { ser, state } => {
                match state {
                    State::Empty => {}
                    _ => tri!(ser.formatter.end_array(&mut ser.writer).map_err(|e| {
                        PythonSerializerError {
                            message: e.to_string(),
                        }
                    })),
    ...
    fn end(self) -> Result<()> {
        match self {
            Compound::Map { ser, state } => {
                ...
                tri!(ser.formatter.end_object(&mut ser.writer).map_err(|e| {
                    PythonSerializerError {
                        message: e.to_string(),
                    }
                }));
        ...
        match self {
            Compound::Map { ser, state } => {
                ...
                *state = State::Rest;
                ...
                tri!(ser.formatter.end_object_key(&mut ser.writer).map_err(|e| {
                    PythonSerializerError {
                        message: e.to_string(),
                    }
                }));
    ...
    fn end(self) -> Result<()> {
        match self {
            Compound::Map { ser, state } => {
                match state {
                    State::Empty => {}
                    _ => tri!(ser.formatter.end_object(&mut ser.writer).map_err(|e| {
                        PythonSerializerError {
                            message: e.to_string(),
                        }
                    })),
