...
| 15 | Model config in JSON Schema generation | Read from the model class's `model_config` (`title`, `extra`, `json_schema_extra`, `json_schema_mode_override`, …); the class docstring becomes the `description`; callables (`json_schema_extra` functions, `model_title_generator`, `pydantic_js_functions`) customise the schema | Read from the `model` schema's `config`, with core config names (`title`, `extra_fields_behavior`, `str_min_length`, `ser_json_bytes`, `validate_by_alias`) plus pydantic's `json_schema_extra` (dict only), `json_schema_mode_override` and `json_schema_serialization_defaults_required`; descriptions come from `metadata.pydantic_js_updates`; callables are a schema error | The core has no class objects; the host passes what pydantic reads off the class as schema data |
