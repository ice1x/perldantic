...
subtest 'validate_python and validate_json' => sub {
    ...
    $e = dies { $ints->validate_python(['1'], strict => 1) };
...
subtest 'types holding models' => sub {
    ...
    my $points = $path->validate_python([{x => 1, y => '2'}, Geo::Point->new(x => 0, y => 0)]);
    isa_ok $points->[0], 'Geo::Point';
    is $points->[0]->y, 2;
    is $path->dump_python($points), [{x => 1, y => 2}, {x => 0, y => 0}];
...
subtest 'a model class' => sub {
    ...
    isa_ok $p, 'Geo::Point';
    is $adapter->dump_python($p, exclude => ['y']), {x => 1};
...
subtest 'dumping' => sub {
    ...
    is $maybe->dump_python({a => undef, b => 1.5}), {a => undef, b => 1.5};
...
subtest 'models declared after the adapter was built' => sub {
    ...
    is $adapter->validate_python([{a => 1}])->[0]->a, 1;
    Geo::Late::has(b => (is => 'ro', isa => Int, required => 1));
    my $e = dies { $adapter->validate_python([{a => 1}]) };
...
subtest 'usage errors' => sub {
    for my $case (
        ...
        [sub { Perldantic::TypeAdapter->new(Int)->validate_python(1, 'strict') },
            'validate_python takes options as key => value pairs'],
    ) {
        my $e = dies { $case->[0]->() };
        isa_ok $e, 'Perldantic::UsageError';
        is $e->message, $case->[1];
