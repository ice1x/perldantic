...
subtest 'the Uuid type' => sub {
    ...
    my $u = $ta->validate_python($TEXT);
    isa_ok $u, 'Perldantic::Uuid';
    is "$u", $TEXT;
    is $ta->validate_python(Perldantic::Uuid->new($TEXT))->as_string, $TEXT, 'UUID objects pass';
    ...
    $e = dies { $strict->validate_python($TEXT) };
    ...
    $e = dies { $v4->validate_python($TEXT) };
    ...
    is [map {"$_"} @{Perldantic::TypeAdapter->new(ArrayRef[Uuid])->validate_python([$TEXT, uc $TEXT])}],
        [$TEXT, $TEXT], 'nested';
};
