...
use Perldantic::Types qw(Set Int Str);
...
subtest 'Set[] keeps distinct items in order' => sub {
    ...
    is $ta->validate([3, '1', 3, 1.0, 2]), [3, 1, 2], 'duplicates by value are dropped';
    is $ta->validate_json('[1, 1, 2]'), [1, 2];
    is $ta->dump_json([1, 2]), '[1,2]';
    ok no_warnings { $ta->dump([1, 2]) }, 'Perl arrays serialize as sets without warnings';
    is $ta->json_schema, {type => 'array', uniqueItems => !!1, items => {type => 'integer'}};
    my $e = dies { $ta->validate([1, 'x']) };
    is $e->errors->[0]{loc}, [1];
    is $e->errors->[0]{type}, 'int_parsing';
    $e = dies { $ta->validate('abc') };
...
subtest 'unhashable items and lengths' => sub {
    ...
    my $e = dies { $any->validate([[1], 2]) };
    is $e->errors->[0]{type}, 'set_item_not_hashable';
    my $strs = Set[Str];
    my $short = Perldantic::TypeAdapter->new($strs->with(max_length => 2));
    $e = dies { $short->validate([qw(a b c)]) };
    is $e->errors->[0]{type}, 'too_long';
    ok lives { $short->validate([qw(a a b b)]) }, 'the limit applies to distinct items';
    $e = dies { Perldantic::TypeAdapter->new($strs->with(min_length => 2))->validate([qw(a a)]) };
...
subtest 'strict mode' => sub {
    ...
    is $strict->validate([1, 2, 1]), [1, 2], 'a Perl array is a set even in strict mode';
    ok !Perldantic::Types->can('FrozenSet'), 'no FrozenSet: in Perl it would be the same as Set';
